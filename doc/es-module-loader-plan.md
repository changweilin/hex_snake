...
| D. Service module migration | 依 export map 順序讓 leaf services 與 runtime helpers 改成 explicit imports，legacy loader 仍可回退；`stats.js`、`about.js`、`network.js`、`audio.js` 與 `characters.js` 已完成 service dependency alias slices，集中 `Stats*` / `About*` / `Net*` / `Audio*` / `Character*` aliases | build、quick、smoke、mobile、network、`audit:esm-map` |
