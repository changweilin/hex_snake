...
| C. Dual-mode runtime / registry exports | platform/runtime、state registry、DOM facade、network/about leaf services 與 catalog/media/stats shell 已具備正式 named exports，module shadow 已 import 這些 shell；下一步檢查 runtime helpers shell exports | `audit:esm-map`、`audit:globals` 不上升 |
...
下一個 AI 可直接處理項目是 Phase C 下一段：檢查 `src/replay.js` / `src/ai.js` / `src/render.js` 是否可安全提供 dual-mode shell exports；先只補 facade exports 與 audit contract，module shadow entry 仍不得啟動 gameplay。
