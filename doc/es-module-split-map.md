...
- `npm.cmd run audit:globals`：297 cross-file reads，Phase 4 已先把 `replay.js` 的直接 DOM/runtime reads 與 `render.js` 首批 DOM / board-shake runtime reads 收斂到 `HexSnakeDOM`、`HexSnakeState`、`HexSnakeUI`。
...
| Runtime helpers | `src/ai.js`, `src/render.js`, `src/replay.js` | `replay.js` 首批已改走 DOM/state/UI facade；`render.js` 首批 DOM 與 board-shake state 已改走 facade；`ai.js` 與 `render.js` 剩餘 helper/config reads 仍需分批處理 | 先完成 runtime helper facade，再分批處理 render helper/config 與 AI |
...
| 4. Runtime cleanup | 首批持續完成 | `replay.js` 已改走 `HexSnakeDOM`、`HexSnakeState.game/ui/replay` 與 `HexSnakeUI` facade；`render.js` 的 canvas/playArea/mobileInputQuery、rafId、board-shake state 與 visualLoadScale cells read 已改走 facade；剩餘 audit 項多為 render helper/config state、AI rules reads 或 record schema 欄位 | `audit:globals` 339 -> 297；`audit:state-boundary` 維持 0/0 |
...
1. Phase 4 下一批：續處理 `render.js` 的 helper/config state reads，例如 colors、food type catalog、character catalog、selected attack/target reads。
