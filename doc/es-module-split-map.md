...
- `src/main.js` 仍使用 legacy concatenated loader 載入 13 個 source files，尚未切到正式 module scope；`doc/es-module-export-map.md`、`doc/es-module-loader-plan.md` 與 `npm.cmd run audit:esm-map` 已固定 registry 初始化順序、loader mode/fallback 與 export surface。
...
| --- | --- | --- | --- |
| Foundation | `src/platform/web.js`, `src/platform/mobile.js`, `src/state.js`, `src/dom.js` | `state.js` 與 `dom.js` 沒有 detected cross-file globals；web/mobile platform 已註冊 `HexSnakeRuntime = { platform, storage }`，一般模組改走 `HexSnakeRuntime.platform/storage`；export map 與 loader plan gate 已建立 | 先維持 script-compatible window facade；下一步新增 module shadow entry |
...
| Runtime helpers | `src/ai.js`, `src/render.js`, `src/replay.js` | replay 已改走 DOM/state/UI facade 與 `HexSnakeUI.replayGame`；render draw/public hooks 已改走 `HexSnakeRender` 與 `HexSnakeRenderGame`；AI helper hooks 已改走 `HexSnakeUI.aiGame`；runtime adapter 改走 `HexSnakeRuntime` | 下一步不再新增零散 facade，先新增不啟動 gameplay 的 module shadow entry |
| Core knot | `src/ui.js`, `src/game.js` | `ui.js -> dom.js` 與 `game.js -> dom.js` 都已收斂成 `HexSnakeDOM`；`ui.js -> game.js` 已改走 `HexSnakeUI.uiGame`；`game.js -> service/render/AI/runtime` 已改走 facade；registry init/export map 與 loader plan 已文件化並可驗證 | 最後拆；先用 module shadow entry 驗證 loader plumbing |
...
| 5. Core ES module split | loader split plan gate 建立完成 | 已新增 `doc/es-module-export-map.md`、`doc/es-module-loader-plan.md` 與 `audit:esm-map`；下一步新增 `module-shadow` entry，保留 legacy loader fallback，不直接 import 尚未 dual-mode 的 gameplay files | `audit:esm-map`、legacy loader 可回退；正式 loader 可逐步啟用 |

## Immediate AI Task Queue

1. 新增 module shadow entry 與最小 loader flag 設計：保留 legacy default，不 import 尚未 dual-mode 的 gameplay files。
