...
| Runtime helpers | `src/ai.js`, `src/render.js`, `src/replay.js` | `replay.js` 首批已改走 DOM/state/UI facade，game lifecycle helpers 已改走 `HexSnakeUI.replayGame`；`render.js -> ui.js` direct reads 已清空，`render.js -> ai.js` 已收斂為 `HexSnakeAI`，`render.js -> game.js` 已收斂為 `HexSnakeRenderGame`；`ai.js -> ui.js` direct reads 已清空，`ai.js -> game.js` 已收斂為 `HexSnakeUI.aiGame` | 下一步檢查 `ui.js -> game.js` 與 render draw/public hooks |
...
1. Phase 4 下一批：檢查 `ui.js -> game.js` facade 與 render draw/public hooks 是否需要更窄 facade。
2. 接著整理 `ui.js -> audio/render/game.js`、`game.js -> ai/replay/stats/about.js` 的剩餘 facade 依賴，評估正式 ESM export 形狀。
