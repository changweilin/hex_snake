...
| Core knot | `src/ui.js`, `src/game.js` | `ui.js` 已 export `uiCore` / `HexSnakeUICore`，並集中 `Ui*` dependency aliases；`game.js` 已 export `gameShell` / `HexSnakeGame`、`loadGameShell()` 與 `bootstrapGame()`，集中 `Game*` dependency aliases，control-profile、saved character choice、keybind、attack button highlight、settings / perf overlay、settings clamp / GM parameters 與 GM presets / settings action slices 已由 `audit:esm-map` 固定改走本地 aliases；`main-module.js` 已提供 `loadModuleGame()` 作為正式 module bootstrap owner | 下一步接續 board geometry helper slice，不切 production default |
...
1. 延續 Phase D service module migration：接續 `game.js` board geometry helper slice，並維持 production `bundled-legacy-fallback`。
