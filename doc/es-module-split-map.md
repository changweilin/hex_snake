...
- `npm.cmd run audit:globals`：123 cross-file reads，Phase 4 已先把 `replay.js` 的直接 DOM/runtime reads、`render.js` 對 `ui.js` 的直接 DOM/runtime/config-catalog reads、`ai.js` 的規則/runtime/helper reads 收斂到 facade，並排除 legacy audit 的 object key / local binding / regex literal 誤判。
...
| Catalog / media / stats | `src/characters.js`, `src/audio.js`, `src/stats.js` | 直接 state / DOM reads 已收斂；`audio.js` 角色音效查詢已改走 `HexSnakeUI.characterFor`；`stats.js` 目前只依賴 storage/state/dom 與 game facade | 第二階段首輪完成，後續只需在正式 ESM 前確認 catalog function exports |
| Runtime helpers | `src/ai.js`, `src/render.js`, `src/replay.js` | `replay.js` 首批已改走 DOM/state/UI facade；`render.js -> ui.js` direct reads 已清空，只剩對 state/dom/ui facade 與 game/ai helpers 的依賴；`ai.js -> ui.js` direct reads 已清空，只剩對 state/ui facade 與 game facade 的依賴 | 下一步收斂 `ui.js -> dom.js`，降低正式拆 core knot 的 DOM 壓力 |
| Core knot | `src/ui.js`, `src/game.js` | `ui.js -> game.js` 目前只剩 `HexSnakeGame` facade read；`game.js -> ui.js` 無 detected consumers；`game.js -> dom.js` 直接 DOM globals 已收斂成單一 `HexSnakeDOM` | 最後拆；下一步分批降低 `ui.js -> dom.js` 剩餘 DOM refs |
...
1. Phase 4 下一批：分批處理 `ui.js -> dom.js` DOM refs，先挑 HUD/status/settings 類可直接改走 `HexSnakeDOM` 的項目。
2. 接著檢查 `characters.js -> game.js` 與 `render.js -> ai/game.js` helper 依賴是否需要更窄 facade。
