...
下一個 AI 可直接處理項目是延續 Phase D service module migration：挑下一個 leaf service、runtime helper 或 core shell，把現有 window/facade read 改成 explicit import friendly 的小切片；`ui.js`、`stats.js`、`about.js`、`network.js`、`audio.js`、`characters.js`、`replay.js`、`ai.js` 與 `render.js` 已完成 alias slices，production default 繼續維持 `bundled-legacy-fallback`。
