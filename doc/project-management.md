...
| 架構整理 | Phase 4 dependency 精修完成 | `replay.js`、`render.js`、`ai.js` 已分批改走 facade；`audit:globals` 已排除物件鍵 / local binding / regex literal 誤判，`game.js -> ui.js` 與 `ui.js` detected consumers 清空；下一步分批收斂 `ui.js -> dom.js` DOM refs | `npm run audit:globals`、`npm run audit:state-boundary`、`doc/state-boundary-audit.md`、`doc/es-module-split-map.md` |
...
| 項目 | 狀態 | 下一步 | 完成標準 |
| --- | --- | --- | --- |
| 核心 facade / ES modules | Phase 4 dependency 精修完成 | replay、render、ai 與小型 catalog/about 依賴已分批收斂，`game.js -> ui.js` 與 `ui.js` detected consumers 清空，`audit:globals` 降至 123；下一步處理 `ui.js -> dom.js` DOM refs | build、quick、smoke、audit 通過 |
...
6. Facade / ES modules 排在共用規則核心之前，因為先清楚模組邊界，再抽共享純函式，回歸風險較低；目前 `ui.js`、`render.js`、`ai.js` 可安全移動的執行期 game API 已完成，`running`、`paused`、`gameOver`、match collections、combat/resource state、runtime/session state、角色/方向/timer state、controls/settings state、presentation/actions API、presentation state、character catalog API、misc/render runtime state、attack/input pointer state、read-only config/helper facade、state-boundary audit cleanup、ES module split dependency map、Phase 1 module borders、Phase 2 helper extraction、game.js DOM facade、Phase 3 catalog/media/stats cleanup、Phase 4 replay cleanup 首批、Phase 4 render cleanup 第三批、Phase 4 ai cleanup 首批與 dependency 精修已完成，下一步分批收斂 `ui.js -> dom.js` DOM refs。
...
4. 執行 ES module split 下一批：分批處理 `ui.js -> dom.js` DOM refs，優先從 HUD/status/settings 這類可直接改走 `HexSnakeDOM` 的項目開始。
