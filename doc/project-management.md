...
| ES module Phase D ai root state alias slice | 2026-05-21 `src/ai.js` 將 config/game state 來源拆成 `AiRootState` / `AiConfig` / `AiState`，避免直接以 `HexSnakeState.config/game` 作為 AI helper state alias | `audit:globals` 維持 44，`audit:state-boundary` 維持 0/0；下一輪可套用到 render state alias |
