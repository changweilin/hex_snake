...
| 架構整理 | attack/input pointer state 完成 | 下一輪處理 read-only config/constants API：`defaultSettings`、`defaultKeybinds`、`directions`、`foodTypes`、attack/resource helpers | `npm run audit:globals`、`npm run audit:state-boundary`、`doc/state-boundary-audit.md` |
...
| 項目 | 狀態 | 下一步 | 完成標準 |
| --- | --- | --- | --- |
| 核心 facade / ES modules | attack/input pointer state 完成 | 攻擊按鈕高亮、選擇 profile、棋盤/控制盤 pointer、攻擊按鈕 pointer 與 best timers 已改走 `HexSnakeState`；direct writes 已清零 | build、quick、smoke、audit 通過 |
...
4. 繼續 `game.js` boundary：direct writes 已清零，下一刀處理 read-only config/constants API，優先收斂 `defaultSettings`、`defaultKeybinds`、`directions`、`foodTypes`、攻擊/資源 helper 讀取。
