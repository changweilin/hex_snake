...
| 架構整理 | Phase 3 catalog/media/stats 首輪完成 | `characters.js`、`audio.js`、`stats.js` 的直接 state / DOM reads 已改走 `HexSnakeState`、`HexSnakeUI`、`HexSnakeDOM`；下一步進 Phase 4，先處理 `replay.js` 的 DOM/runtime state reads | `npm run audit:globals`、`npm run audit:state-boundary`、`doc/state-boundary-audit.md`、`doc/es-module-split-map.md` |
...
| 項目 | 狀態 | 下一步 | 完成標準 |
| --- | --- | --- | --- |
| 核心 facade / ES modules | Phase 3 首輪完成 | catalog/media/stats 的直接 state / DOM reads 已收斂，`audit:globals` 降至 339；下一步分批處理 `replay.js` DOM/runtime state | build、quick、smoke、audit 通過 |
...
6. Facade / ES modules 排在共用規則核心之前，因為先清楚模組邊界，再抽共享純函式，回歸風險較低；目前 `ui.js`、`render.js`、`ai.js` 可安全移動的執行期 game API 已完成，`running`、`paused`、`gameOver`、match collections、combat/resource state、runtime/session state、角色/方向/timer state、controls/settings state、presentation/actions API、presentation state、character catalog API、misc/render runtime state、attack/input pointer state、read-only config/helper facade、state-boundary audit cleanup、ES module split dependency map、Phase 1 module borders、Phase 2 helper extraction、game.js DOM facade 與 Phase 3 catalog/media/stats cleanup 已完成，下一步進 Phase 4 replay/runtime cleanup。
...
4. 執行 ES module split Phase 4：先處理 `replay.js` 的 DOM/runtime state reads，再回頭釐清 `game.js -> ui.js` 剩餘設定/runtime state reads。
