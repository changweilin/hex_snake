...
| ES module Phase D game move stick helper alias slice | 2026-05-22 `src/game.js` 的 moveStick / setMoveStickLocked / clear move timers / engageMoveStick / releaseMoveStick 流程改走 `GameRuntimeState` alias，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | `audit:globals` 維持 44，`audit:state-boundary` 維持 0/0；下一輪可接續 target stick helper direct facade reads |
