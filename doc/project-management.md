...
| ES module Phase D game keyboard aim key helper alias slice | 2026-05-22 `src/game.js` 的 handleKeyboardAimKeyDown / handleKeyboardAimKeyUp / clearKeyboardAimKeyLocks 流程改走 `GameRuntimeState` alias，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 player direct attack helper alias slice |
| ES module Phase D game player direct attack helper alias slice | 2026-05-22 `src/game.js` 的 playerDirectAttackTarget / playerGestureAttackDirection / playerDirectAttackOptions / previewDirectAttack / launchDirectPlayerAttack 流程改走 `GameRuntimeState`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 player attack failure helper alias slice |
| ES module Phase D game player attack failure helper alias slice | 2026-05-22 `src/game.js` 的 playerAttackFailureReason 流程改走 `GameRuntimeState`、`GameConfig`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 launch player attack helper alias slice |
| ES module Phase D game launch player attack helper alias slice | 2026-05-22 `src/game.js` 的 launchPlayerAttack 流程改走 `GameRuntimeState`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 launch player attack direction helper alias slice |
| ES module Phase D game launch player attack direction helper alias slice | 2026-05-22 `src/game.js` 的 launchPlayerAttackDirection 流程改走 `GameRuntimeState` alias，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 perform module attack helper alias slice |
| ES module Phase D game perform module attack helper alias slice | 2026-05-22 `src/game.js` 的 performModuleAttack 流程改走 `GameRuntimeState` alias，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 clear module hold helper alias slice |
| ES module Phase D game clear module hold helper alias slice | 2026-05-22 `src/game.js` 的 clearModuleHold 流程改走 `GameRuntimeState` alias，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 toggle pause helper alias slice |
| ES module Phase D game toggle pause helper alias slice | 2026-05-22 `src/game.js` 的 togglePause 流程改走 `GameRuntimeState`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 surrender game helper alias slice |
| ES module Phase D game surrender game helper alias slice | 2026-05-22 `src/game.js` 的 surrenderGame 流程改走 `GameRuntimeState` alias，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 board attack pointer helper alias slice |
| ES module Phase D game board attack pointer helper alias slice | 2026-05-22 `src/game.js` 的 board attack pointer 流程改走 `GameRuntimeState` alias，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 settings panel helper alias slice |
| ES module Phase D game settings panel helper alias slice | 2026-05-22 `src/game.js` 的 settings / GM / network panel helper 流程改走 `GameRuntimeState` alias，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 platform back button helper alias slice |
| ES module Phase D game platform back button helper alias slice | 2026-05-22 `src/game.js` 的 handlePlatformBackButton 流程改走 `GameRuntimeState`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 portrait interaction helper alias slice |
| ES module Phase D game portrait interaction helper alias slice | 2026-05-22 `src/game.js` 的 winner portrait / portrait lightbox interaction 流程改走 `GamePresentationState`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 setup input helper alias slice |
| ES module Phase D game setup input helper alias slice | 2026-05-22 `src/game.js` 的 grid / food / difficulty / initial resource / character setup input listeners 改走 `GameRuntimeState`、`GameConfig`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 reset best time / settings action button alias slice |
| ES module Phase D game reset best time / settings action button alias slice | 2026-05-22 `src/game.js` 的 reset best time、GM preset buttons、reset settings button 流程改走 `GameRuntimeState`、`GameConfig`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 attack button pointer / keyboard aim button alias slice |
| ES module Phase D game attack button pointer / keyboard aim button alias slice | 2026-05-22 `src/game.js` 的 defaultPlayerAttackTarget、attack button pointer 與 keyboard aim button pointer handlers 改走 `GameRuntimeState`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 rules / tutorial launcher alias slice |
| ES module Phase D game rules / tutorial launcher alias slice | 2026-05-22 `src/game.js` 的 rules open/close、tutorial launch card 與 rules backdrop close 流程改走 `GameUI` alias，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 keybind input alias slice |
| ES module Phase D game keybind input alias slice | 2026-05-22 `src/game.js` 的 keybind input change handlers 改走 `GameRuntimeState` alias，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | 已接續完成 intro / tutorial interaction alias slice |
| ES module Phase D game intro / tutorial interaction alias slice | 2026-05-22 `src/game.js` 的 intro close、tutorial action click 與 tutorial swipe pointer handlers 改走 `GamePresentationState`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | `audit:globals` 維持 44，`audit:state-boundary` 維持 0/0；下一輪可接續 start button direct facade reads |
