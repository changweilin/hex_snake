...
| 架構整理 | Phase 4 runtime adapter facade 收斂完成 | 新增 `HexSnakeRuntime`，將 `HexSnakePlatform` / `HexSnakeStorage` 正式整理成單一 runtime adapter facade；一般模組改從 `HexSnakeRuntime.platform/storage` 取用；`audit:globals` 降至 42；下一步確認 `HexSnakeState` / `HexSnakeDOM` / `HexSnakeUI` registry 的 ESM 初始化順序 | `npm run audit:globals`、`npm run audit:state-boundary`、`doc/state-boundary-audit.md`、`doc/es-module-split-map.md` |
...
| 項目 | 狀態 | 下一步 | 完成標準 |
| --- | --- | --- | --- |
| 核心 facade / ES modules | Phase 4 runtime adapter facade 收斂完成 | replay、render、ai 與小型 catalog/about 依賴已分批收斂，角色 helper 讀取已改走 `HexSnakeUI`，AI helper 讀取已改走 `HexSnakeUI.ai`，render game helper 讀取已改走 `HexSnakeRenderGame`，replay/AI/UI game hooks 已改走 `HexSnakeUI.replayGame`、`HexSnakeUI.aiGame`、`HexSnakeUI.uiGame`，render public hooks 已改走 `HexSnakeRender`，platform/storage 已改走 `HexSnakeRuntime`；`audit:globals` 降至 42 | build、quick、smoke、audit 通過 |
...
6. Facade / ES modules 排在共用規則核心之前，因為先清楚模組邊界，再抽共享純函式，回歸風險較低；目前 `ui.js`、`render.js`、`ai.js` 可安全移動的執行期 game API 已完成，`running`、`paused`、`gameOver`、match collections、combat/resource state、runtime/session state、角色/方向/timer state、controls/settings state、presentation/actions API、presentation state、character catalog API、misc/render runtime state、attack/input pointer state、read-only config/helper facade、state-boundary audit cleanup、ES module split dependency map、Phase 1 module borders、Phase 2 helper extraction、game.js DOM facade、Phase 3 catalog/media/stats cleanup、Phase 4 replay cleanup 首批、Phase 4 render cleanup 第三批、Phase 4 ai cleanup 首批、dependency 精修、ui DOM facade 首批、helper facade 收斂、render/helper hooks 收斂、replay game hook 收斂、ai game hook 收斂、ui/render public hooks 收斂、service facade 收斂與 runtime adapter facade 收斂已完成，下一步確認 `HexSnakeState` / `HexSnakeDOM` / `HexSnakeUI` registry 在正式 ESM 下的初始化順序。
...
4. 執行 ES module split 下一批：確認 `HexSnakeState` / `HexSnakeDOM` / `HexSnakeUI` registry 在正式 ESM 下的初始化順序，補齊可直接驗證的 export map 文件。
