...
| ES module Phase D game start game entry alias slice | 2026-05-21 `src/game.js` 的 startGame entry / startup setup 流程改走 `GameRuntimeState`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | `audit:globals` 維持 44，`audit:state-boundary` 維持 0/0；下一輪可接續 auto start game slice |
