...
| ES module Phase D game network helper alias slice | 2026-05-21 `src/game.js` 的 LAN snapshot / start / input / guest snapshot helper 流程改走 `GameRuntimeState`、`GameConfig`、`GameUI` aliases，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | `audit:globals` 維持 44，`audit:state-boundary` 維持 0/0；下一輪可接續 sandworm status helper direct facade reads |
