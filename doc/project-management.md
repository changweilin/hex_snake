...
| ES module Phase D game settings perf overlay alias slice | 2026-05-21 `src/game.js` 的 left-hand / low-power / perf overlay 流程改走 `GamePresentationState` alias，並以 `audit:esm-map` 固定該 slice 不再直接讀 `HexSnakeState.` / `HexSnakeUI.` | `audit:globals` 維持 44，`audit:state-boundary` 維持 0/0；下一輪可接續 settings clamp / GM parameters slice |
