...
| 架構整理 | 角色/方向/timer state 完成 | 下一輪處理 controls/settings state 首輪：grid/food/speed/GM settings、keybinds、keyboard aim/preview、pointer state | `npm run audit:globals`、`npm run audit:state-boundary`、`doc/state-boundary-audit.md` |
...
| 項目 | 狀態 | 下一步 | 完成標準 |
| --- | --- | --- | --- |
| 核心 facade / ES modules | 角色/方向/timer state 完成 | character choice/id、computer difficulty、dir/nextDir/computerDir、energy/bomb flashes、hold/preview timers 已改走 `HexSnakeState.game`；下一輪處理 controls/settings state | build、quick、smoke、audit 通過 |
...
| 角色/方向/timer state | 2026-05-20 將 character choice/id、computer difficulty、dir/nextDir/computerDir、energy/bomb flashes、hold/preview timers 包成 `HexSnakeState.game` getter/setter；`audit:globals` 從 660 降至 645，`audit:state-boundary` 從 939/197 降至 817/182 | 下一輪處理 controls/settings state 首輪：grid/food/speed/GM settings、keybinds、keyboard aim/preview、pointer state |
...
6. Facade / ES modules 排在共用規則核心之前，因為先清楚模組邊界，再抽共享純函式，回歸風險較低；目前 `ui.js`、`render.js`、`ai.js` 可安全移動的執行期 game API 已完成，`running`、`paused`、`gameOver`、match collections、combat/resource state、runtime/session state 與角色/方向/timer state 已接到 `HexSnakeState.game`，下一步處理 controls/settings state。
...
4. 繼續 `game.js` state boundary：下一刀處理 controls/settings state 首輪，優先收斂 grid/food/speed/GM settings、keybinds、keyboard aim/preview、pointer state。
