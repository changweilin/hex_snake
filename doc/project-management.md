...
| 架構整理 | controls/settings state 首輪完成 | 下一輪處理 presentation/actions API：portrait、tutorial、rules、result share、callouts、character stage | `npm run audit:globals`、`npm run audit:state-boundary`、`doc/state-boundary-audit.md` |
...
| 項目 | 狀態 | 下一步 | 完成標準 |
| --- | --- | --- | --- |
| 核心 facade / ES modules | controls/settings state 首輪完成 | grid/food/speed/GM settings、keybinds、keyboard aim/preview、pointer target state 已改走 `HexSnakeState.game`；下一輪處理 presentation/actions API | build、quick、smoke、audit 通過 |
...
| Controls/settings state 首輪 | 2026-05-20 將 grid/food/speed/GM settings、keybinds、keyboard aim/preview、target 與 joystick pointer state 包成 `HexSnakeState.game` getter/setter；`audit:globals` 從 645 降至 631，`audit:state-boundary` 從 817/182 降至 668/168 | 下一輪處理 presentation/actions API：portrait、tutorial、rules、result share、callouts、character stage |
...
6. Facade / ES modules 排在共用規則核心之前，因為先清楚模組邊界，再抽共享純函式，回歸風險較低；目前 `ui.js`、`render.js`、`ai.js` 可安全移動的執行期 game API 已完成，`running`、`paused`、`gameOver`、match collections、combat/resource state、runtime/session state、角色/方向/timer state 與 controls/settings state 首輪已接到 `HexSnakeState.game`，下一步處理 presentation/actions API。
...
4. 繼續 `game.js` boundary：下一刀處理 presentation/actions API，優先收斂 portrait、tutorial、rules、result share、callouts、character stage。
