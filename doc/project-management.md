...
| 架構整理 | presentation/actions API 首輪完成 | 下一輪處理 presentation state：result share、tutorial / portrait swipe、logo / replay record timers 等剩餘狀態 | `npm run audit:globals`、`npm run audit:state-boundary`、`doc/state-boundary-audit.md` |
...
| 項目 | 狀態 | 下一步 | 完成標準 |
| --- | --- | --- | --- |
| 核心 facade / ES modules | presentation/actions API 首輪完成 | portrait、tutorial、rules、result share、callouts、character stage 動作已改走 `HexSnakeUI`；下一輪處理剩餘 presentation state | build、quick、smoke、audit 通過 |
...
| Presentation/actions API 首輪 | 2026-05-20 建立 `HexSnakeUI` facade，將 `game.js` 對 portrait、tutorial、rules、result share、callouts、character stage 的 UI 動作呼叫收斂到單一入口；`audit:globals` 從 631 降至 600，`audit:state-boundary` 從 668/168 降至 564/135 | 下一輪處理 presentation state：result share、tutorial / portrait swipe、logo / replay record timers |
...
6. Facade / ES modules 排在共用規則核心之前，因為先清楚模組邊界，再抽共享純函式，回歸風險較低；目前 `ui.js`、`render.js`、`ai.js` 可安全移動的執行期 game API 已完成，`running`、`paused`、`gameOver`、match collections、combat/resource state、runtime/session state、角色/方向/timer state、controls/settings state 與 presentation/actions API 已接到明確邊界，下一步處理剩餘 presentation state。
...
4. 繼續 `game.js` boundary：下一刀處理剩餘 presentation state，優先收斂 result share、tutorial / portrait swipe、logo transition 與 replay record timing。
