...
- Phase D 已開始 service module migration 小切片；`state.js` controls storage、`ui.js` controls helper / registry self-reads、`stats.js` / `audio.js` / `replay.js` / `ai.js` root state、`about.js`、`network.js`、`characters.js`、`render.js` 與 `game.js` 已集中 dependency aliases，後續可逐步替換成 explicit imports。
