...
- Phase D 已開始 service module migration 小切片；`state.js` controls storage、`ui.js` controls helper / registry self-reads、`stats.js` / `audio.js` / `replay.js` / `ai.js` / `render.js` root state、`about.js`、`network.js`、`characters.js` 與 `game.js` 已集中 dependency aliases，後續可逐步替換成 explicit imports。
...
下一個 AI 可直接處理項目是延續 Phase D service module migration：`game.js` control-profile、saved character choice、keybind、attack button highlight、settings / perf overlay、settings clamp / GM parameters、GM presets / settings action、board geometry helper、reset game setup、result share helper、start logo countdown 與 skip logo transition slices 已改走本地 `Game*` aliases 並加上 `audit:esm-map` preflight；下一輪可接續 start game entry slice，production default 繼續維持 `bundled-legacy-fallback`。
