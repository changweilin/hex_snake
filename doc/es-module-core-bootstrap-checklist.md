...
下一個 AI 可直接處理項目是延續 Phase D service module migration：`game.js` control-profile、saved character choice 與 keybind slices 已改走本地 `Game*` aliases 並加上 `audit:esm-map` preflight；下一輪可接續 attack button highlight slice，production default 繼續維持 `bundled-legacy-fallback`。
