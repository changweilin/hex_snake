...
- 已完成：新增設定配置檔，可保存、套用與刪除鍵位、方向鍵、左手模式、按鍵攻擊目標、角色選擇、電腦難度、GM 開局參數、音效靜音、低耗電與 FPS 顯示設定，資料保存於 storage adapter。
- 已完成：desktop/mobile smoke 會保存設定配置、改動小招鍵與開局設定、reload 後套用恢復、刪除配置，確認既有操作與設定可回復。
